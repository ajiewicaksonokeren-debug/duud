...
- **Artikel Berhadiah** — CMS artikel (Admin → Artikel) yang memberi tiket roulette + koin/XP saat dibaca sampai selesai. Timer baca dihitung di server (bukan di HP pemain) berdasarkan estimasi jumlah kata, jadi tidak bisa diklaim instan; sebagian artikel juga punya kuis singkat 1 pertanyaan yang wajib dijawab benar dulu sebelum reward cair. Reward hanya bisa diklaim sekali per artikel per akun selamanya.
- **Tebak Skor MPL** — prediksi skor pertandingan esports (mis. MPL) sebelum kick-off untuk memenangkan tiket roulette (Admin → Tebak Skor untuk kelola jadwal & hasil). Prediksi otomatis terkunci begitu jam pertandingan tiba (dicek dari jam server, bukan jam HP pemain) dan admin men-"settle" hasil akhir setelah pertandingan usai untuk membagikan reward secara otomatis & sekali jalan (tidak bisa disettle dua kali).
...
- **Artikel** (`/api/articles`, tabel `articles` & `article_reads`): `POST /:id/start` membuat sesi baca (session token + timestamp mulai, keduanya disimpan di server) lalu `POST /:id/complete` mengecek dari sisi server apakah waktu yang berlalu sudah memenuhi `min_read_seconds` (dihitung otomatis dari jumlah kata artikel) dan, kalau artikel punya kuis, apakah jawabannya benar — permintaan klaim yang datang sebelum waktunya atau dengan jawaban salah ditolak. Reward hanya diberikan sekali per (user, artikel) lewat constraint `UNIQUE` di database. IP (di-hash, bukan disimpan mentah) dicatat per baca dan dibandingkan di `/api/articles/admin/flagged` untuk menandai pola mencurigakan (banyak akun berbeda menyelesaikan artikel yang sama dari IP yang sama) supaya admin bisa tinjau manual.
- **Tebak Skor** (`/api/esports`, tabel `esports_matches` & `match_predictions`): `POST /matches/:id/predict` hanya diterima kalau `Date.now()` di server masih sebelum `match_time` pertandingan tersebut — jam yang dikirim dari HP pemain tidak pernah dipakai untuk validasi ini. Satu akun hanya bisa punya satu prediksi aktif per pertandingan (boleh diubah selama belum terkunci). Admin men-settle skor akhir lewat `POST /admin/matches/:id/settle`, yang dijalankan dalam satu transaksi database dan diberi guard `settled` supaya tidak mungkin membagikan reward dua kali untuk pertandingan yang sama.
- **Rate limiting & anti-cheat umum**: `server/src/utils/antiCheat.js` menyediakan rate limiter in-memory (dipakai di endpoint start/complete artikel dan predict tebak skor) serta fungsi hashing IP (bergaram, tidak pernah menyimpan IP asli) yang dipakai untuk deteksi pola mencurigakan di kedua fitur di atas.
